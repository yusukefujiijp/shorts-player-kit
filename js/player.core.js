...
function renderPlaceholder(scene){
  ...
  if(scene && scene.base) setBg(scene.base); // base 未指定なら既定テーマのまま
...
function renderContent(scene){
  ...
  (function renderSectionTags(){
    // 正規化：配列優先（sectionTags）、なければ従来の単数（sectionTag）
    const raw = Array.isArray(scene.sectionTags)
      ? scene.sectionTags
      : (scene.sectionTag ? [scene.sectionTag] : []);
    const tags = (raw||[])
      .map(x => String(x||'').trim())
      .filter(x => x.length>0);
