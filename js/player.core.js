...
(() => {
  ...
  function finalizeStopIfNeeded(context){ if(Ctrl.stopRequested && !Ctrl.stopped){ Ctrl.stopped = true; const t = nowMs(); const lat = Math.max(0, Math.round(t - (Ctrl.stopReqAt||t))); try{ window.dispatchEvent(new CustomEvent('player:stop-confirm', { detail:{ latencyMs: lat, context: String(context||'') } })); }catch(_){} } }
  ...
  function speakStrict(text, rate = rateFor('narr'), role='narr'){
    return new Promise(async (resolve)=>{
      ...
      let settled=false, started=false, fallbackTried=false; const done=()=>{ if(!settled){ settled=true; resolve(); } };
  ...
  async function playScene(scene){
    ...
    switch(kind){
      ...
      case 'effect':
        if(State.playingLock) break; State.playingLock = true;
        try{ renderEffect(scene); const raw=(scene.t ?? scene.duration ?? scene.durationMs ?? scene.effectDuration ?? 1200); const ms=Math.max(0, Math.min(60000, Number(raw)||1200)); await sleep(ms); } finally { State.playingLock = false; }
