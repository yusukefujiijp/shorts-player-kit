...
(() => {
	...
	function setBg(c) {
		try {
			...
			ensureBgLayer().style.backgroundColor = c;
		} catch (_) {}
	}
	...
	function chooseVoice(role) {
		const vm = window.__ttsVoiceMap || {},
			map = vm[role];
		if (map) {
			if (typeof map === 'string') { const v = voiceById(map); if (v) return v; } else if (map && typeof map === 'object') {
				try { if (typeof SpeechSynthesisVoice !== 'undefined' && map instanceof SpeechSynthesisVoice) return map; } catch (_) {}
				const key = map.voiceURI || ((map.lang || '') + '|' + (map.name || ''));
	...
	async function speakOrWait(text, rate = rateFor('narr'), role = 'narr') {
		...
		if (!cleaned) return;
		const eff = effRateFor(role, rate);
		if (TTS_ENABLED) { const parts = splitChunksJa(cleaned); for (const p of parts) { await speakStrict(p, eff, role); } } else {
	...
	// --- TTS sanitize bridge (dynamic import; HTML改修不要) ---
	let __ttsSanModule = null;
	async function __getNarrForTTS(scene) {
		try {
			__ttsSanModule = __ttsSanModule || await import('./tts_sanitize.js');
			if (__ttsSanModule && typeof __ttsSanModule.getTtsText === 'function') {
				return __ttsSanModule.getTtsText(scene); // narrTTS優先/なければ絵文字除去
			}
		} catch (_) { /* フォールバックへ */ }
		// フォールバック：既存クリーニングに委ね（少なくともサロゲート絵文字は除去）
		// 旧: return String(scene && (scene.narrTTS || scene.narr) || '');
