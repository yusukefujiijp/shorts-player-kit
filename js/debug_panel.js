...
(function() {
	...
	var SECTIONS = Object.assign({ status: true, note: false, controls: true, goto: true, ttsFlags: true, voices: true, baseRate: false },
		(CFG_IN.sections || {})
	);
	var BUTTONS = Object.assign({ prev: true, next: true, play: false, stop: false, restart: true, goto: true, hardreload: true, hardstop: false },
		(CFG_IN.buttons || {})
	);
	...
	var QUICKBAR = Object.assign({ enabled: true, mode: 'twoRows', items: { play: true, stop: true, next: true, ack: true } },
		(CFG_IN.quickbar || {})
	);
	...
	(function initInsetSync() {
		...
		// パネル自身のサイズ変化
		try { ro = new ResizeObserver(schedule);
			ro.observe(host); } catch (_) {}
	...
	*/
	...
	function renderVoices() {
		...
		// 件数を見出し内に表示（Voices [9 voices]）
		var cnt = list.length;
		var note = h('span', 'voices-note', '[' + cnt + ' voices]');
