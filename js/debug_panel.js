...
(function(){
  ...
  // ===== Quick Bar (2 rows固定) + Expanded Body =====
  host.innerHTML =
    '<div class="dbg-bar">'+
      '<div class="qb-row qb-row1">'+
        '<button id="dbg-toggle" class="dbg-toggle" title="展開/折り畳み">🐞 Debug <span id="dbg-arrow"></span></button>'+
        (QUICKBAR.items.play? '<button data-act="play" class="qb-btn play">▶︎</button>':'')+
        (QUICKBAR.items.stop? '<button data-act="stop" class="qb-btn stop">■</button>':'')+
        (QUICKBAR.items.ack?  '<span id="qb-ack" class="qb-ack" aria-live="polite" aria-atomic="true"></span>':'')+
      '</div>'+
      '<div class="qb-row qb-row2"><span id="dbg-status" class="dbg-status">Ready.</span></div>'+
    '</div>'+
    '<div id="dbg-body" class="dbg-body">'+
      '<div id="dbg-statechips" class="statechips" aria-hidden="false"></div>'+
  ...
  var body=$('#dbg-body'), tgl=$('#dbg-toggle'), arrow=$('#dbg-arrow');
  var statusEl=$('#dbg-status'), gotoInp=$('#dbg-goto');
  var ackEl=$('#qb-ack'), chipsEl=$('#dbg-statechips');
  ...
  function updateAck(mode){
    if(!ackEl) return;
    var reserved = '<span class="qb-ack__inner"></span>'; // 幅を一定に保つ器
    if(mode==='pending'){
      ackEl.className = 'qb-ack is-pending';
      ackEl.innerHTML = '<span class="qb-dot"></span> Stopping…'+reserved;
    }else if(mode==='stopped'){
      ackEl.className = 'qb-ack is-stopped';
      ackEl.innerHTML = '<span class="qb-dot"></span> Stopped '+(stopAck.latencyMs|0)+'ms'+reserved;
      clearTimeout(ackTimer);
      ackTimer = setTimeout(function(){ ackEl.className='qb-ack'; ackEl.innerHTML = reserved; }, 1600);
    }else{
      ackEl.className = 'qb-ack';
      ackEl.innerHTML = reserved;
    }
  }
  updateAck('none');
  ...
  /* ========================= Status Polling ===================== */
  function renderStateChips(ss){
    if(!chipsEl) return;
    if(BADGE_MOTION==='off'){
      chipsEl.innerHTML =
        '<span class="dbg-badge speaking'+(ss.speaking?' on':'')+'">speaking</span>'+
        '<span class="dbg-badge paused'  +(ss.paused  ?' on':'')+'">paused</span>'+
        '<span class="dbg-badge pending' +(ss.pending ?' on':'')+'">pending</span>';
      return;
    }
    var pulse = (BADGE_MOTION==='auto') ? ' pulse' : '';
    chipsEl.innerHTML =
      '<span class="dbg-badge speaking'+(ss.speaking?' on':'')+pulse+'">speaking</span>'+
      '<span class="dbg-badge paused'  +(ss.paused  ?' on':'')+pulse+'">paused</span>'+
      '<span class="dbg-badge pending' +(ss.pending ?' on':'')+pulse+'">pending</span>';
  }
