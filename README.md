# shorts-player-kit — *AI-New Era Playbook v3* (iOS-first)
...
## 🫀 Heartbeat（空コミット代替の“接触記録”）
<!-- APPEND-ONLY: heartbeat (single line + recent touches) -->
- **Last touched (JST)**: 2025-09-12 00:00 JST
- *使い方*: 差分が無い日も **上行の時刻だけを更新**して Commit → Push。Working Copy でも空コミット相当が実現できる。

**Recent touches（newest-first / 参考サンプルを2行常設）**  
- 2025-09-12 00:00 JST — 小粒コミットの脈拍を刻む（例）  
- 2025-09-11 06:19 JST — Pages/OG検証ログを追記（例）
...
> p = "README.md"; tgt = "*Last touched (JST)*:"
...
1. **決定事項の更新** → [0. いまの決定事項](#0-いまの決定事項source-of-truth)  
   1.1 上部＝**最新スナップショット（上書き可）**／下部＝**Decision History（oldest-first）**。  
2. **Pages公開の作業記録** → [5. Pages デプロイ](#5-pages-デプロイgithub-actions) の **Deployment Journal（newest-first）**。  
3. **OG画像/ファビコン等の資産更新** → [6. アセット運用](#6-アセット運用og画像favicon-他) の **Assets Journal（newest-first）**。  
4. **日次の到達点・学び** → [10. 日次チェックポイント](#10-日次チェックポイントappend-only) の **Daily（newest-first）**。  
5. **不具合や対処メモ** → [7. トラブル対処集](#7-トラブル対処集faq--troubleshooting)。  
6. **次の一手（ロードマップ）** → [9. フェーズ別ロードマップ](#9-フェーズ別ロードマップone-phase-per-day)（番号順＝**oldest-first**）。
...
### Decision History（Append-only / oldest-first｜サンプル2行を常設）
<!-- APPEND-ONLY: decisions (oldest-first) -->
- 2025-09-01: TTS規範を「役割別の絶対レート」に統一（例）  
- 2025-09-08: 既定レートを 1.4 に確定、UIは Role Rate × 専用に（例）
...
Working Copy は空コミット不可。Heartbeat の時刻更新または各ジャーナル先頭に1行追加で小差分を作る。
...
5. Pages デプロイ（GitHub Actions）
	...
	•	成功判定: ActionsのRunが緑Success かつ Deploy to GitHub Pages が緑。
	•	URL: https://yusukefujiijp.github.io/shorts-player-kit/。

Deployment Journal（Append-only / newest-first｜サンプル2行を常設）

<!-- APPEND-ONLY: deploy (newest-first) -->


	•	2025-09-12: .gitignore 強化／404を絶対パス化（復帰導線を固定）
...
Assets Journal（Append-only / newest-first｜サンプル2行を常設）

<!-- APPEND-ONLY: assets (newest-first) -->


	•	2025-09-12: OG画像の配色/余白を微調整（SNS可読性を向上）（例）
	•	2025-09-10: OG画像とTwitter画像のURLを統一／SVGファビコンをSPK版に最適化（例）

⸻

7. トラブル対処集（FAQ & Troubleshooting）
	•	404（公開URL）: index.html がリポ直下か、.github/workflows/static.yml の path: '.' を確認。
...
<!-- APPEND-ONLY: roadmap (oldest-first) -->


	•	2025-09-12: Phase 1 実装 → Pages直叩き検証まで完了（例）
	•	2025-09-10: Phase 0 確認（Pages成功／SNSメタOK）（例）

⸻

10. 日次チェックポイント（Append-only / newest-first｜サンプル2行を常設）

書式: YYYY-MM-DD: <一行の到達点> — <学び/課題/次アクション>

<!-- APPEND-ONLY: daily (newest-first) -->


	•	2025-09-12: .gitignoreと404を強化し、公開導線を固定 — 次は Day7 差替えの下準備（例）
	•	2025-09-10: Pages公開/OG/Twitterカード検証を記録 — 次は .gitignore と 404 を整備（例）
...
11. SNSメタタグ検証ログ（完成判定の根拠）
	...
	•	整合性: canonical / og:url / og:image / twitter:image の完全一致。
